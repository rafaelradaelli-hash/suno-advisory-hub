...
/* ── Inline Supabase helpers (matches App.jsx credentials) ── */
...
var TONE_MAP = {
  simples: "TOM SIMPLES: escreva para alguém que nunca investiu. Sem termos técnicos (P/L, EBITDA, yield, DY, FFO, spread). Use analogias do dia a dia. Frases curtas.",
  intermediario: "TOM INTERMEDIÁRIO: cliente que investe há alguns anos. Termos populares liberados (dividendo, rendimento, cotação, CDI). Termos avançados (DY, FFO) explicados brevemente na primeira vez.",
  profissional: "TOM PROFISSIONAL: linguagem técnica completa. Use livremente: DY, FFO, ABL, vacância, cap rate, NOI, LTV, CRI, CRA, IPCA+. Tom de relatório de research institucional.",
};
  ...
  return (
    <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
      ...
      {TONE_OPTIONS.map(function(t) {
        ...
        return (
          <button key={t.key} onClick={function() { p.onChange(t.key); }}
...
export default function FIIsPage() {
  ...
  function openNew() { setEditingFii(null); setForm(EMPTY_FORM); setError(""); setModalOpen(true); }
    ...
    <div style={{ padding: "20px 24px", maxWidth: "900px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "11px", color: "rgba(255,255,255,0.35)" }}>Atualizações extraídas dos relatórios gerenciais</p>
        </div>
        <button onClick={openNew} style={{ background: "#DC2626", color: "#fff", border: "none", borderRadius: "8px", padding: "9px 18px", fontWeight: 700, fontSize: "12px", cursor: "pointer" }}>
          + Adicionar FII
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "80px 0", color: "rgba(255,255,255,0.15)" }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "11px" }}>Clique em "Adicionar FII" para começar.</p>
        </div>
      ) : (
      ...
      {modalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: "16px" }}>
          <div style={{ background: "#111", borderRadius: "16px", border: "1px solid rgba(220,38,38,0.2)", width: "100%", maxWidth: "680px", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <h2 style={{ margin: 0, fontSize: "16px", fontWeight: 800, color: "#fff" }}>{editingFii ? "Atualizar " + editingFii.ticker : "Adicionar FII"}</h2>
              <button onClick={function() { setModalOpen(false); }} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.4)", fontSize: "20px", cursor: "pointer" }}>✕</button>
            </div>

            <div style={{ padding: "18px 22px", overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "12px" }}>
              {error && <div style={{ background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: "8px", padding: "10px 14px", fontSize: "11px", color: "#f87171" }}>{error}</div>}
              ...
              <div>
                ...
                <textarea style={Object.assign({}, iS, { resize: "vertical", minHeight: "220px", lineHeight: 1.6 })}
                  placeholder="Cole aqui o texto completo do relatório gerencial do FII..."
                  value={form.raw_text}
            ...
            <div style={{ padding: "14px 22px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
              <button onClick={function() { setModalOpen(false); }} style={{ padding: "9px 20px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(255,255,255,0.5)", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
              <button onClick={processAndSave} disabled={processing || !form.ticker.trim() || !form.raw_text.trim()}
                style={{ padding: "9px 22px", borderRadius: "8px", border: "none", background: (!processing && form.ticker.trim() && form.raw_text.trim()) ? "#DC2626" : "rgba(255,255,255,0.05)", color: (!processing && form.ticker.trim() && form.raw_text.trim()) ? "#fff" : "rgba(255,255,255,0.3)", fontSize: "12px", fontWeight: 700, cursor: processing ? "wait" : "pointer" }}>
